...
//Chart caption
$caption = "Stock Availability Rate at District Store";
...
<div class="widget widget-tabs">    
	...
	<?php 
        ...
        if(!empty($stakeholder))
            $qry_1 .= " AND stakeholder.stkid in($stakeholder) ";
...
";
        ...
        //Query for shipment main dashboard
         $qry = "
            select stkid,stkname,sum(sufficient) as total_so_dist,count(all_items) as itm_count
            FROM(
                SELECT
                  stakeholder.stkid,
                  stakeholder.stkname,
                  summary_district.item_id,(CASE WHEN summary_district.soh_district_store/avg_consumption > 0 THEN 1 ELSE 0 END) as sufficient,
                  count(distinct summary_district.district_id) as all_items,
        ...
        if(!empty($stakeholder))
            $qry .= " AND stakeholder.stkid in($stakeholder) ";
 ...
 ";
    ...
    while($row = mysql_fetch_assoc($qryRes))
    {
       //print_r($row);
        ...
        $perc1 = ((!empty($row['total_so_dist'])?$row['total_so_dist']:0)* 100)/$row['itm_count'];
        $r_arr[$row['stkid']] = $perc1;
    }  
    
    //echo '<pre>';print_r($r_arr);print_r($stk_arr);exit;    
        
    //xml for chart
    $xmlstore = '<chart caption="'.$caption.' - '.$prov_name.' - '.$dist_name.'" yaxismaxvalue="100" subcaption="Stakeholder Wise" xaxisname="Stakeholders" yaxisname="Percentage" numberprefix="" palettecolors="#26C281" exportEnabled="1" theme="fint">';
    ...
    foreach($stk_arr as $id => $name)
    {
        ...
        //$xmlstore .= '     <set value="'.(number_format(!empty($val)?$val:0,1)).'" link="JavaScript:showDrillDown_b1('.$province.',\''.$prov_name.'\',\''.$from_date.'\','.$id.',\''.$name.'\');" />';
         $xmlstore .= '     <set label="'.$name.'" value="'.(number_format(!empty($val)?$val:0,1)).'" link="JavaScript:showDrillDown_dist_b1('.$province.',\''.$prov_name.'\',\''.$from_date.'\','.$id.',\''.$name.'\',\''.$dist.'\',\''.$dist_name.'\');"  tooltext="'.$name.':'.(number_format(!empty($val)?$val:0,1)).' {br} Click bar to drill down"  />';
