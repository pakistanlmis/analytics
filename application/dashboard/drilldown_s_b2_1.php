...
//Chart caption
$caption = "Stock Avaialability Status ";
...
<div class="widget widget-tabs">    
    ...
    //xml for chart
    $xmlstore = '<chart caption="Average Stock Availability Rate at SDP Level - '.$stk_name.' - '.$prod_name.'" yaxismaxvalue="100"  subcaption="" xaxisname="Products" yaxisname="Percentage" numberprefix="" theme="fint">';
   
    //$xmlstore .= ' <dataset seriesname="Average Stock Availabiilty Rate at SDP Level">';
    foreach($so_arr as $dist_id => $val)
    {
        ...
        $xmlstore .= '     <set label="'.$dist_arr[$dist_id].'" value="'.(number_format($perc  , 2)).'"  link="JavaScript:showDrillDown_lvl2('.$province.',\''.$prov_name.'\',\''.$from_date.'\','.$funding_source.',\''.$stk_name.'\',\''.$prod_id.'\',\''.$prod_name.'\',\''.$dist_id.'\',\''.$dist_arr[$dist_id].'\');"  />';
    }
    //$xmlstore .= '  </dataset>';
...
<script>
     function showDrillDown_lvl2(prov,prov_name,from_date,stk,stk_name,prod_id,prod_name,dist_id,dist_name) {
        var div_id = "drilldown_div_2";
        var url = 'drilldown_s_b2_2.php';
